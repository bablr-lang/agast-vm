...
export class NodePropertiesFacade {
  ...
  at(name, index = -1, shiftIndex = 0) {
    ...
    if (hasOwn(properties, name)) {
      ...
      if (binding.reference.value.flags.expression) {
        return NodeFacade.from(btree.getAt(shiftIndex + 1, binding.node));
      } else {
        return NodeFacade.from(binding.node);
      }
    }
    return null;
  }
  ...
  referenceAt(name, index = -1) {
    return getProperties(buildPathSegment(name, index), nodes.get(this))?.reference;
  }
