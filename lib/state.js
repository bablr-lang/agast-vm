...
export const StateFacade = class AgastStateFacade {
  ...
  get referencePath() {
    let { referencePath } = actuals.get(this);
    return referencePath && new TagPathFacade(referencePath);
  }
...
export const State = class AgastState {
  ...
  static from(held) {
    return new State(held && { node: held, path: Path.from(held) });
  }
    ...
    switch (tag.type) {
      ...
      case OpenNodeTag: {
        ...
        if (!this.path) {
          ...
          this.path = this.path.push(node, sumtree.getSize(parentNode.children) - 1);
      ...
      case EmbeddedNode: {
        ...
        if (this.held) {
          let matchesHeld =
            this.held.node === tag.value || getFirstProperty(tag.value) === this.held.node;
          if (matchesHeld) {
            this.held = null;
          } else {
            throw new Error();
      ...
      case ShiftTag: {
        ...
        const finishedPath = this.resultPath.inner;
