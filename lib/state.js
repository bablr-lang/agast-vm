...
export const State = class AgastState {
  ...
  advance(tag) {
    ...
    switch (tag.type) {
      ...
      case ReferenceTag: {
        ...
        if (name && getOr(0, name, this.node) === undefined) {
          let existingReferenceIndex = getInitializerTagsIndex(this.node, tag);
          let existingReferenceTag = Tags.getAt(existingReferenceIndex, this.node.tags);
            ...
            if (count === 1) {
              ...
              if (!property && nodeHas(key, this.node)) {
                ...
                if (!ref.value.isArray) {
                  let reference = Tags.getAt(refIndex, this.node.tags).value;
      ...
      case InitializerTag: {
        ...
        if (this.held && Tags.getAt(2, node.tags)?.type === InitializerTag) {
          throw new Error();
        }
    ...
    this.resultPath = targetPath && TagPath.from(targetPath, -1, tag.type === Property ? -1 : null);
