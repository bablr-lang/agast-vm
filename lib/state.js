  ...
  advance(tag) {
    ...
    switch (tag.type) {
      ...
      case ReferenceTag: {
        ...
        if (tag.value.name) {
          if (
            hasOwn(this.node.properties, tag.value.name) &&
            !referencesAreEqual(tag.value, this.node.properties[tag.value.name].reference)
          ) {
        ...
        if (this.node.type) {
          for (const value of Object.values(this.node.properties)) {
            if (!Array.isArray(value) && value.node === undefined) {
              addProperty(
                this.node,
                buildProperty(value.reference, buildBinding(), buildNullNode()),
              );
            }
          }
        }
