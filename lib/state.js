  ...
  advance(tag) {
    ...
    switch (tag.type) {
      ...
      case CloseNodeTag: {
        ...
        let { lastPropertyTagPath } = this.path;

        let closeStack = this.path.node.bounds[1];
        ...
        this.path.node.bounds = buildBounds(this.path.node.bounds[0], closeStack);
        ...
        if (this.path) {
          ...
          let isFirstProperty =
            this.held || lastTagPath.propertyWrapper.equalTo(this.path.firstPropertyTagPath);

          let openStack = this.path.node.bounds[0];
          if (isFirstProperty) {
            openStack = BTree.replaceAt(
              -1,
              openStack,
              lastTagPath.propertyWrapper.child.value.property,
            );
          ...
          this.path.node.bounds = buildBounds(openStack, this.path.node.bounds[1]);
        }
        break;
      }
