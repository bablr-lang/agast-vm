    ...
    switch (tag.type) {
      ...
      case ReferenceTag: {
        ...
        if (tag.value.name) {
          ...
          if (
            rootIdx != null &&
            !referencesAreEqual(tag.value, Tags.getAt(rootIdx, this.node.tags).value)
          ) {
            throw new Error('mismatched references');
          }
        }
        ...
        if (name && getOr(0, name, this.node) === undefined) {
          let initReferenceIndex = getInitializerTagsIndex(this.node, tag);
          if (!initReferenceIndex) throw new Error('double reference');
          let initReferenceTag = Tags.getAt(initReferenceIndex, this.node.tags);
      ...
      case BindingTag: {
        ...
        let refTag = this.referenceTag;

        let tagProperty = buildProperty(refTag.value, tag.value, this.held ?? undefined);
        ...
        this.node.tags = Tags.replaceAt(
          ...
          this.node.tags,
          buildChild(PropertyWrapper, buildPropertyWrapper([refTag, tag], tagProperty)),
        );
        break;
      }
      ...
      case OpenNodeTag: {
        ...
        let bindingTag = parentNode && Tags.getAt(-1, parentNode.tags);
        ...
        if (!this.path) {
          ...
          this.path = this.path.push(node, Tags.getSize(parentNode.tags) - 2);
      ...
      case CloseNodeTag: {
        ...
        if (this.node.type) {
          for (const { 0: key, 1: count } of Object.entries(
            Tags.getSums(this.node.tags).references,
          )) {
            if (count === 1) {
              ...
              if (!property && nodeHas(key, this.node)) {
                ...
                let ref = Tags.getAt(refIndex, this.node.tags);
      ...
      case NullTag: {
        ...
        targetPath = this.path.push(stubNode, Tags.getSize(this.node.tags) - 3);
        break;
      }
