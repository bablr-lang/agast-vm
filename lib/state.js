  ...
  advance(tag) {
    ...
    switch (tag.type) {
      ...
      case OpenFragmentTag: {
        ...
        this.node.type = null;
        this.node.language = this.node.attributes.bablrLanguage;
      ...
      case GapTag: {
        ...
        if (ref && ref.type !== ReferenceTag) throw new Error();
        ...
        if (this.held) {
          ...
          this.node.type = Symbol.for('@bablr/gap');
          target = this.node;
        }
      ...
      case ShiftTag: {
        ...
        if (ref.value.isArray) {
          ...
          properties[ref.value.name] = btree.replaceAt(-1, properties[ref.value.name], {
            reference: ref,
            node: buildStubNode(buildNullTag()),
          });
          const { childrenIndexes } = this.path;
          childrenIndexes[ref.value.name].pop();
        } else {
          properties[ref.value.name] = null;
        }

        this.node.children = btree.pop(this.node.children);
        this.path.referenceIndexes.pop();
