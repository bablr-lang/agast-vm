    ...
    switch (tag.type) {
      ...
      case GapTag: {
        if (s.path) {
          ...
          s.path = Path.from(buildNode(printTag(tag)));
          s.resultPath = s.path.tagPathAt(-1);
        }
      ...
      case OpenNodeTag: {
        s.path = !s.path ? Path.fromTag(rawTag) : s.path.advance(rawTag);
      ...
      case Property: {
        ...
        for (tag of BList.traverse(tags)) {
