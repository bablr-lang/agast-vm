...
import { isValidTag, parseTag, parseTagType } from '@bablr/agast-helpers/builders';
...
function* __agast(s) {
  ...
  for (;;) {
    ...
    switch (tag.type) {
      ...
      case BindingTag: {
        const { segments } = parseTag(tag).value;
      ...
      case OpenNodeTag: {
        ...
        if (parseTag(tag).value.selfClosing) {
      ...
      case Property: {
        ...
        for (tag of Tags.traverse(tags)) {
          if (!isValidTag(tag) && !isKnownValid(tag)) throw new Error();
          s.path = s.path.advance(tag);
        }
