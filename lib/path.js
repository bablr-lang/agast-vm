...
export class TagPathFacade {
  static from(path, tagsIndex) {
    return path && new TagPathFacade(TagPath.from(paths.get(path), tagsIndex));
  }

  static fromNode(node, tagsIndex) {
    let path = node && TagPath.fromNode(nodes.get(node), tagsIndex);
...
export class PathFacade {
  ...
  tagPathAt(tagsIndex) {
    return TagPathFacade.wrap(TagPath.from(paths.get(this), tagsIndex));
  }
