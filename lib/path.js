...
export const Path = class AgastPath extends WeakStackFrame {
  ...
  static pushTag(context, path, tag) {
    switch (tag.type) {
      case 'DoctypeTag': {
        let rootPath = Path.from(context);

        if (path) {
          throw new Error();
        }

        context.tagPaths.set(tag, rootPath);

        rootPath.range[0] = tag;

        rootPath = rootPath.push(context, buildReference('root', false));
  ...
  get language() {
    return this.depth ? this.startTag.value?.language : null;
  }

  get type() {
    return this.depth ? this.startTag.value.type : Symbol.for('@bablr/fragment');
  }

  get isToken() {
    return this.depth ? this.startTag.value.flags?.token : false;
  }
