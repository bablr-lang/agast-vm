...
export class TagPathFacade {
  static from(path, tagsIndex, offsetIndex) {
    return path && new TagPathFacade(TagPath.from(paths.get(path), tagsIndex, offsetIndex));
  }

  static fromNode(node, tagsIndex, offsetIndex) {
    let path = node && TagPath.fromNode(nodes.get(node), tagsIndex, offsetIndex);
    ...
    this.tag =
      tag.type === Property
        ? buildChild(
            Property,
            buildFacadeProperty(
              tag.value.property.reference,
              tag.value.property.binding,
              new NodeFacade(tag.value.property.node),
            ),
          )
        : tag;
  ...
  equalTo(tagPath) {
    ...
    let { path, tagsIndex } = paths.get(this);
    let tagPath_ = paths.get(tagPath);
    return path.node === tagPath_.path.node && tagsIndex === tagPath_.tagsIndex;
  }
}
...
export class PathFacade {
  ...
  tagPathAt(tagsIndex, offsetIndex) {
    return TagPathFacade.wrap(TagPath.from(paths.get(this), tagsIndex, offsetIndex));
  }

  tagAt(tagsIndex) {
    return this.tagPathAt(tagsIndex).tag;
  }
